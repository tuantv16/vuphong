    ...
    <div class="panel panel-default frame564175037f8b9a474e8b4567 frameOptionb9ac4e8a7c panel-article-right panel-article-detail" id="frameBoundB581c23695bef7">
        <div class="panel-heading" style="background: #0071a6; color: white; font-weight: bold; text-transform: uppercase; min-height: 40px;">
            <div class="panel-title">
            		...
            		@if(isset($dataProcess[0]->cat_id))
						{{ $dataProcess[0]->category_nm }}
            		@endif
        ...
        <div class="panel-body">
            <div id="module22" class="ModuleWrapper" modulerootid="3090929">
                <section id="section22" class="section-list Article-Detail-listType4">
                	@if(isset($dataProcess[0]->post_id))
                	@foreach($dataProcess as $key => $row)
	                	<?php 
	                		$date = date_create($row->category_updater_date);
                        ...
                        <div class="post-item row clearfix">
                            <div class="col-xs-12 col-sm-5 col-md-4 left-listType4">
                                <figure class="post-image">
                                    <div class="mask"></div>
                                    <a href="{{ asset("$row->cat_slug/$row->post_slug-$row->post_id.html")}}" title="{{$row->post_slug}}">
                                    	<?php 
                                    		if(file_exists( public_path().'/uploads/'.$row->filename) && $row->filename != '' ) {
                                    	?>          	
                                    		<img class="post-image relative-height" data-size-ratio="3:2" width="100%" src='{{URL::asset("/uploads/$row->filename")}}' alt="{{$row->post_slug}}" itemprop="image" style="height: 161px;">
										<?php } else { ?>
												<img class="post-image relative-height" data-size-ratio="3:2" width="100%" src='{{URL::asset("/uploads/noimage/no-image-news.png")}}' alt="{{$row->post_slug}}" itemprop="image" style="height: 161px;">
										<?php } ?>
                            ...
                            <div class="col-xs-12 col-sm-7 col-md-8">
                                <div class="post-title" style="margin-top:-10px">
                                    <h5 class="entry-title" itemprop="headline">
                                    	<a href="{{ asset("$row->cat_slug/$row->post_slug-$row->post_id.html")}}">{{$row->title}}</a>
                                    </h5>
                                </div>
                    ...
                    <?php echo ($key != count($dataProcess)-1)?'<hr/>':''?>
          			...
          			<div class="text-center">
		                {{ $dataProcess->appends(['sort' => 'post.id'])->links() }}
		            </div>    
